<div align="center">

<img
  width="100%"
  src="https://capsule-render.vercel.app/api?type=waving&color=0:4F46E5,50:2563EB,100:06B6D4&height=220&section=header&text=Doeun%20Kim&fontSize=46&fontColor=FFFFFF&fontAlignY=38&animation=fadeIn&desc=Backend%20Developer&descSize=20&descAlignY=58"
/>

<a href="https://github.com/kimSummerrain">
  <img
    src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=21&duration=3000&pause=900&color=3B82F6&center=true&vCenter=true&width=760&lines=Java+%26+Spring+Boot+Backend+Developer;Backend+%7C+Cloud+%7C+DevOps;Software+%2B+Hardware+Integration;Building+Reliable+Backend+Systems"
    alt="Typing Introduction"
  />
</a>

<br/>

<img
  src="https://komarev.com/ghpvc/?username=kimSummerrain&label=Profile%20Views&color=2563eb&style=flat-square"
/>

</div>

---

## 👋 About Me

```yaml
Name: Doeun Kim
Role: Backend Developer
Location: South Korea
Education: B.S. in Convergence Software
```

- Backend developer focused on **Java & Spring Boot**
- Experience with **AWS, Docker, CI/CD, and Linux server environments**
- Built systems involving **AI APIs, asynchronous processing, and real-time communication**
- Experience integrating **backend software with drones and embedded hardware**
- Interested in building **reliable and scalable backend systems**

---

## 💻 Tech Stack

### Languages

<div align="center">

<img src="https://skillicons.dev/icons?i=java,python,javascript&theme=dark" />

</div>

<br/>

### Backend

<div align="center">

<img src="https://skillicons.dev/icons?i=spring,django,flask&theme=dark" />

<br/><br/>

<img src="https://img.shields.io/badge/REST_API-005571?style=flat-square" />
<img src="https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white" />
<img src="https://img.shields.io/badge/WebSocket-010101?style=flat-square" />
<img src="https://img.shields.io/badge/SSE-4F46E5?style=flat-square" />

</div>

<br/>

### Database & Cache

<div align="center">

<img src="https://skillicons.dev/icons?i=mysql,postgres,redis,sqlite&theme=dark" />

</div>

<br/>

### Cloud & DevOps

<div align="center">

<img src="https://skillicons.dev/icons?i=aws,docker,githubactions,nginx,linux&theme=dark" />

</div>

<br/>

### Tools

<div align="center">

<img src="https://skillicons.dev/icons?i=git,github,idea,vscode,postman&theme=dark" />

</div>

<br/>

### Hardware & Systems

<div align="center">

<img src="https://skillicons.dev/icons?i=raspberrypi,linux&theme=dark" />

<br/><br/>

<img src="https://img.shields.io/badge/Pixhawk-Flight_Controller-2563EB?style=flat-square" />
<img src="https://img.shields.io/badge/MAVLink-UAV_Communication-06B6D4?style=flat-square" />
<img src="https://img.shields.io/badge/UAV-Autonomous_Drone-475569?style=flat-square" />

</div>

---

## 🚀 Experience

### Backend Developer Intern

**AI Content Generation Platform**

`Spring Boot` · `JWT` · `AWS` · `Docker` · `GitHub Actions`

- Developed backend APIs and integrated external AI services
- Improved polling and state management for asynchronous AI generation jobs
- Implemented AWS S3 file handling and cloud infrastructure
- Built CI/CD workflows using GitHub Actions and AWS ECR

<br/>

### Smart Agriculture & Autonomous Drone System

`Django` · `Redis` · `WebSocket` · `Raspberry Pi` · `Pixhawk` · `MAVLink`

- Built real-time communication between autonomous drones and backend servers
- Integrated drone telemetry, image transmission, and server-side processing
- Co-authored research presented at **KIIT Conference 2025**
- **Gold Award — Engineering Festival**

<br/>

### 🌐 Backend & Web Services

`Spring Boot` · `Flask` · `AWS` · `Docker` · `SSE`

- Built and deployed multiple team-based backend services
- Developed a real ordering service used during a university festival
- Experience with REST APIs, authentication, real-time communication, and deployment

---

## 🧠 Algorithm & Problem Solving

<div align="center">

<a href="https://solved.ac/profile/kdeun8485">
  <img
    src="https://mazassumnida.wtf/api/v2/generate_badge?boj=kdeun8485"
    alt="Solved.ac Profile"
  />
</a>

<br/><br/>

`Data Structures` · `Algorithms` · `Graph` · `DFS/BFS`  
`Dynamic Programming` · `Binary Search` · `Problem Solving`

</div>

---

## Achievements

| Achievement | Result |
| :--- | :---: |
| Engineering Festival | 🥇 Gold Award |
| Kairos Hackathon | 🏆 Grand Prize |
| KIIT Conference 2025 | 📄 Research Paper |
| Baekjoon | 🟡 Gold IV |
| SQLD | ✅ Certified |

---

## GitHub

<div align="center">

<img
  src="https://github-readme-stats.vercel.app/api?username=kimSummerrain&show_icons=true&hide_border=true&theme=transparent"
/>

<br/><br/>

<img
  src="https://streak-stats.demolab.com?user=kimSummerrain&hide_border=true&background=00000000&ring=3B82F6&fire=06B6D4&currStreakLabel=3B82F6&sideLabels=64748B&dates=64748B&currStreakNum=64748B&sideNums=64748B"
/>

</div>

---

## 🐾 Contribution

<div align="center">

<a href="https://github.com/devxb/gitanimals">
  <img
    width="100%"
    src="https://render.gitanimals.org/lines/kimSummerrain"
    alt="Gitanimals"
  />
</a>

</div>

---

## 📫 Contact

<div align="center">

<a href="mailto:doeun457@gmail.com">
  <img
    src="https://img.shields.io/badge/Email-doeun457%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white"
  />
</a>

<a href="https://github.com/kimSummerrain">
  <img
    src="https://img.shields.io/badge/GitHub-kimSummerrain-181717?style=for-the-badge&logo=github&logoColor=white"
  />
</a>

</div>

<br/><br/>

<div align="center">

### Building reliable systems, from backend servers to real-world devices.

</div>

<br/>

<img
  width="100%"
  src="https://capsule-render.vercel.app/api?type=waving&color=0:4F46E5,50:2563EB,100:06B6D4&height=110&section=footer"
/>
